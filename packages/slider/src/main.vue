...
export default {
  ...
  watch: {
    value(val, oldVal) {
      if (
        this.dragging || // 在拖动过程中
        (Array.isArray(val) &&
          Array.isArray(oldVal) &&
          val.every((item, index) => item === oldVal[index])) // value与oldValue，都是数组且每一项均相等 => range, range === oldRange
      ) {
        return;
      }
      this.setValues();
    ...
    secondValue() {
      if (this.range) {
        this.$emit('input', [this.minValue, this.maxValue]); // todo minValue, maxValue 与firstValue, secondValue关系？
      }
    },
  ...
  methods: {
    ...
    setValues() {
      ...
      } else if (!this.range && typeof val === 'number' && !isNaN(val)) { // 单btn
        if (val < this.min) {
          this.$emit('input', this.min);
        } else if (val > this.max) {
          this.$emit('input', this.max);
        } else { // v < min, v > max， v 不是有效number, 做了类型和数值边界判断
          this.firstValue = val; // val => firstValue
    ...
    emitChange() {
      this.$nextTick(() => { // ? 为什么在nextTick里面调而不是直接调用？
        this.$emit(
          'change',
  ...
  computed: {
    stops() { // 与marks的区别， stop，如果有step的话，则显示各个间隔点。 mark是在stop基础上标记。
      ...
      const stopCount = (this.max - this.min) / this.step; // 几个间隔点， 不包括 0
      const stepWidth = (100 * this.step) / (this.max - this.min);
      const result = [];
      for (let i = 1; i < stopCount; i++) {
        result.push(i * stepWidth); // result: 百分比数组
      }
      if (this.range) {
        ...
        return result.filter( // 只在蓝色bar的右侧展示间隔点
          step =>
    ...
    markList() {
      ...
      return marksKeys
        .map(parseFloat) // string => number
        .sort((a, b) => a - b) // sort
        .filter(point => point <= this.max && point >= this.min) // boundary
        .map(point => ({
          point,
          position: ((point - this.min) * 100) / (this.max - this.min),
          mark: this.marks[point] // mark可能是要mark的数值，也可能是对象，包含style和label
        }));
    },
    ...
    precision() { // 从min, max, step三者中取精度最高的一个作为精度参考
  ...
  mounted() {
    ...
    if (this.range) {
      ...
      if (typeof this.value !== 'number' || isNaN(this.value)) {
        ...
        this.firstValue = Math.min(this.max, Math.max(this.min, this.value)); // 先将this.value和min, max比较，使其符合边界，再赋值
        // this.value > this.max => this.firstValue = this.max
        // this.value < this.min => this.firstValue = this.min
        // this.firstValue = this.value
      }
      this.oldValue = this.firstValue;
      valuetext = this.firstValue;
    }
    this.$el.setAttribute('aria-valuetext', valuetext);// range与否，valuetext值不同，所以需要额外定义一个变量 valuetext
